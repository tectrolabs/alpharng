 ...
 *
 * Copyright (C) 2014-2025 TectroLabs, https://tectrolabs.com
 *
...
static ssize_t thread_device_read(char *buffer, size_t length)
{
   ...
   if (isEntropySrcRdy) {
      ...
      do {
         ...
         if (retval == SUCCESS) {
            size_t act = TRND_OUT_BUFFSIZE - curTrngOutIdx;
            if (act > (length - total)) {
               act = (length - total);
            }
...
static ssize_t proc_read(struct file *file, char __user *buffer, size_t length, loff_t * offset)
{
   ...
   if (proc_ready_to_read_flag) {
      if (isEntropySrcRdy == false) {
         ...
         len = 30;
         int bytesNotCopied = (int)copy_to_user(buffer, "No AlphaRNG information found\n", len);
         if (bytesNotCopied != 0) {
...
int thread_function(void *data)
{
   ...
   while (!isShutDown) {
      unsigned long waitStatus = wait_for_completion_timeout(&threadData->to_thread_event, msecs_to_jiffies(1000));
      if (waitStatus == 0) {
         continue;
      }
 ...
 *
 */
static int snd_rcv_usb_data(const char *snd, int sizeSnd, char *rcv, int sizeRcv, int opTimeoutSecs)
{
   int retry;
   int actualcCnt = 0;
   int retval = SUCCESS;
...
static int acm_full_read(unsigned char *data, int size, int *bytesTransfered)
{
   ...
   while (totalBytesReceived < size) {
      int bytesReceived = acm_read(acmCtxt->filed, data + totalBytesReceived, size - totalBytesReceived);
      if (bytesReceived < 0) {
            ...
            acm_filldir_callback(void* data, const char *name, int nameLength, loff_t offset, u64 ino, unsigned int dType)
{
   ...
   if (debugMode) {
      pr_info("%s: acm_filldir_callback() - dev_name_by_id: %s \n", DRIVER_NAME, acmCtxt->dev_name_by_id[acmCtxt->devices_found]);
   }
...
static bool acm_device_probe(void)
{
   ...
   for (i = 0; i < acmCtxt->devices_found; ++i) {
      ...
      int op_status = kern_path(acmCtxt->dev_name, LOOKUP_FOLLOW, &acmCtxt->path);
      if (op_status) {
         if (debugMode) {
